...
use crate::metrics::{LIBRA2_JELLYFISH_LEAF_COUNT, LIBRA2_JELLYFISH_LEAF_DELETION_COUNT, COUNTER};
    ...
    K: Key,
{
    ...
    pub fn put_top_levels_nodes(
        ...
        LIBRA2_JELLYFISH_LEAF_COUNT.set(root_node.leaf_count() as i64);
    ...
    K: Key,
{
    ...
    if kvs.len() == 1 && &kvs[0].0 == existing_leaf_key {
        if let (key, Some((value_hash, state_key))) = kvs[0] {
            ...
            LIBRA2_JELLYFISH_LEAF_DELETION_COUNT.inc();
            Ok(None)
        }
    } else {
