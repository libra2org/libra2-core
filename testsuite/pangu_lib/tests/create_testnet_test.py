...
class testnet_tests_create_testnet(unittest.TestCase):
    def test_create_testnet(self) -> None:
        ...
        libra2_cli_path: str = "test_libra2_cli"
        dry_run: bool = False
        ...
        expected_commands = [
            ...
            FakeCommand(
                f"{libra2_cli_path} genesis generate-genesis --local-repository-dir {workspace}/{util.PANGU_WORKSPACE_NAME}/{namespace} --output-dir {workspace}/{util.PANGU_WORKSPACE_NAME}/{namespace}",
                RunResult(0, b"output"),
            ),
        ]
    ...
    def test_generate_genesis(self) -> None:
        ...
        libra2_cli_path: str = "test_aptos"
        ...
        expected_commands = [
            FakeCommand(
                f"{libra2_cli_path} genesis generate-keys --output-dir {user_dir}",
                RunResult(0, b"output"),
            ),
            FakeCommand(
                f"{libra2_cli_path} genesis set-validator-configuration --owner-public-identity-file {workspace}/blueprint1-node-1/public-keys.yaml --local-repository-dir {workspace} --username {username} --validator-host {validator_host} --full-node-host {fullnode_host} --stake-amount {cur_stake_amount}",
                RunResult(0, b"output"),
            ),
            FakeCommand(
            ...
            FakeCommand(
                f"{libra2_cli_path} genesis generate-genesis --local-repository-dir {workspace} --output-dir {workspace}",
                RunResult(0, b"output"),
            ),
        ]
