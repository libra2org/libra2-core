...
use libra2_forge::{reconfig, wait_for_all_nodes_to_catchup, LocalSwarm, NodeExt, Swarm, SwarmExt};
...
async fn assert_reordering(swarm: &mut dyn Swarm, expected_reordering: bool) {
    swarm
        ...
        .await;
    let transaction_factory = swarm.libra2_public_info().transaction_factory();
