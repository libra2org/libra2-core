...
static LIBRA2_VAULT: Lazy<Option<VaultRunner>> = Lazy::new(|| match VaultRunner::run() {
    Err(err) => {
        assert!(
            std::env::var("LIBRA2_REQUIRE_VAULT_TESTS").is_err(),
...
pub fn test_host_safe() -> Option<String> {
    LIBRA2_VAULT.as_ref().map(|v| v.host().to_string())
}
