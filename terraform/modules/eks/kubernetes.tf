...
resource "local_file" "kubernetes" {
  ...
  content = jsonencode({
    kubernetes_host        = aws_eks_cluster.libra2.endpoint
    kubernetes_ca_cert     = base64decode(aws_eks_cluster.libra2.certificate_authority[0].data)
    issuer                 = aws_eks_cluster.libra2.identity[0].oidc[0].issuer
