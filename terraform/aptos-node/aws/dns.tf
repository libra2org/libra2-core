...
data "kubernetes_service" "validator-lb" {
  ...
  metadata {
    # This is the main validator LB service that is created by the libra2-node helm chart
...
data "kubernetes_service" "fullnode-lb" {
  count = var.zone_id == "" || !var.create_records ? 0 : 1
  metadata {
    # This is the main fullnode LB service that is created by the libra2-node helm chart
