...
If you're not familar with GCP (Google Cloud Platform), checkout this [tutorial](https://docs.libra2.org/nodes/full-node/run-a-fullnode-on-gcp#prerequisites) for GCP account setup.

## Run on GCP
This guide assumes you already have GCP account setup, and have created a new project for deploying Libra2 node.
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  module "libra2-node" {
    # download Terraform module from libra2org/libra2-core repo
    source        = "github.com/libra2org/libra2-core.git//terraform/libra2-node/gcp?ref=testnet"
  ...
  For the full customization options, see the variables file [here](https://github.com/libra2org/libra2-core/blob/main/terraform/libra2-node/gcp/variables.tf), and the [helm values](https://github.com/libra2org/libra2-core/blob/main/terraform/helm/libra2-node/values.yaml).
...
12. Create layout YAML file, which defines the node in the validatorSet. For test mode, we can create a genesis blob containing only one node. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Libra2 Labs**
...
13. Download Libra2Framework Move bytecode into a folder named `framework`. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Libra2 Labs**

    Download the Libra2 Framework from the release page: https://github.com/libra2org/libra2-core/releases/tag/libra2-framework-v0.1.0
...
14. Compile genesis blob and waypoint. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Libra2 Labs**
