...
resource "azuread_service_principal" "aptos" {
  application_id = azuread_application.libra2.application_id
}
...
resource "azuread_application_password" "aptos" {
  application_object_id = azuread_application.libra2.object_id
  end_date_relative     = "8760h"
}

resource "azurerm_role_assignment" "subnet" {
  principal_id         = azuread_service_principal.libra2.id
