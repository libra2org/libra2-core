...
def test_init(run_helper: RunHelper, test_name=None):
    ...
    run_helper.run_command(
        test_name,
        ["libra2", "init", "--assume-yes", "--network", "local"],
        input="\n",
    )

    # Assert that the CLI config is there.
    config_path = os.path.join(
        run_helper.host_working_directory, ".libra2", "config.yaml"
    )
    if not os.path.exists(config_path):
        raise TestError(
            f"{config_path} not found (in host working dir) after running libra2 init"
        )
...
@test_case
def test_libra2_header_included(run_helper: RunHelper, test_name=None):
    # Make sure the libra2-cli header is included on the original request
    response = requests.get(run_helper.get_metrics_url())

    if 'request_source_client="libra2-cli' not in response.text:
        raise TestError("Request should contain the correct libra2 header: libra2-cli")
