...
/// 1 LBT_(might not actually get that much, depending on the faucet)
...
pub struct InitTool {
    ...
    ///
    /// Make sure that you have your Ledger device connected and unlocked, with the Libra2 app installed and opened.
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Libra2 already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        let funded = matches!(client
            .get_account_balance(address, "0x1::Libra2Coin::Libra2Coin")
        ...
        eprintln!(
            "\n---\nLibra2 CLI is now set up for account {} as profile {}!\n---\n",
        ...
        if !funded {
            match network {
                ...
                Network::Testnet => {
                    ...
                    eprintln!("The account has not been funded on chain yet. To fund the account and get LBT_on testnet you must visit {}", mint_site_url);
