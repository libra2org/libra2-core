...
impl FromStr for GithubRepo {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            Err(CliError::CommandArgumentError("Invalid repository must be of the form 'owner/repository` e.g. 'libra2org/libra2-core'".to_string()))
        } else {
            Ok(GithubRepo {
...
pub struct GitOptions {
    /// Github repository e.g. 'libra2org/libra2-core'
    ///
