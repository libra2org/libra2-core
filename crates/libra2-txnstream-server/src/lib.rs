...
use libra2_protos::pb::libra2::txnstream::v1::{
...
pub type HealthSvc = tonic_health::server::HealthServer;

pub fn make_health_reporter() -> (HealthReporter, HealthSvc) {
    health_reporter()
}
...
pub async fn serve(addr: &str) -> Result<()> {
    libra2_telemetry::install_simple();

    let addr = addr.parse().context("invalid addr")?;
    let ctx = ServiceContext::from_env().await?;

    let (mut health, health_svc) = make_health_reporter();
