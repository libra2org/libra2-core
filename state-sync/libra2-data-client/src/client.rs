...
pub struct Libra2DataClient {
    ...
    /// The config for the Libra2Net data client.
    data_client_config: Arc<Libra2DataClientConfig>,
    /// The underlying Libra2Net storage service client.
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = Libra2NetResponseCallback {
...
/// The Libra2Net-specific request context needed to update a peer's scoring.
struct Libra2NetResponseCallback {
...
impl ResponseCallback for Libra2NetResponseCallback {
...
impl fmt::Debug for Libra2NetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Libra2NetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
