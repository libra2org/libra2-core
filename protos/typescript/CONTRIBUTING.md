...
1. Run `pnpm publish --dry-run`. From here, make some sanity checks:
  a. Look closely at the output of the command. Pay close attention to what is packaged. Make sure we're not including some files that were included accidentally. For example `.libra2`. Add those to .npmignore if needed.
...
1. Double check that the release worked by visitng npmjs: https://www.npmjs.com/package/libra2-protos.
