...
module libra2_experimental::confidential_asset {
    use std::bcs;
    use std::error;
    ...
    use libra2_framework::object::{Self, ExtendRef, Object};
    ...
    use libra2_experimental::confidential_proof::{
        Self,
        NormalizationProof,
        RotationProof,
        TransferProof,
    ...
    public fun enable_allow_list(libra2_framework: &signer) acquires FAController {
        system_addresses::assert_libra2_framework(libra2_framework);

        let fa_controller = borrow_global_mut<FAController>(@libra2_experimental);
    ...
    public fun disable_allow_list(libra2_framework: &signer) acquires FAController {
        system_addresses::assert_libra2_framework(libra2_framework);
    ...
    public fun enable_token(
        libra2_framework: &signer, token: Object<Metadata>
    ...
    public fun set_auditor(
        libra2_framework: &signer, token: Object<Metadata>, new_auditor_ek: vector<u8>
    ) acquires FAConfig, FAController {
        system_addresses::assert_libra2_framework(libra2_framework);
    ...
    public fun confidential_asset_controller_exists(): bool {
        exists<FAController>(@libra2_experimental)
    }
    ...
    public fun is_allow_list_enabled(): bool acquires FAController {
        ...
        borrow_global<FAController>(@libra2_experimental).allow_list_enabled
    }
    ...
    fun get_fa_store_signer(): signer acquires FAController {
        object::generate_signer_for_extending(
            &borrow_global<FAController>(@libra2_experimental).extend_ref
        )
    }
    ...
    fun get_fa_config_signer(token: Object<Metadata>): signer acquires FAController {
        let fa_ext = &borrow_global<FAController>(@libra2_experimental).extend_ref;
    ...
    entry fun init_module_for_genesis(deployer: &signer) {
        assert!(
            signer::address_of(deployer) == @libra2_experimental,
