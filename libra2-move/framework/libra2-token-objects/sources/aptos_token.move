...
module libra2_token_objects::libra2_token {
    ...
    public fun mint_token_object(
        ...
        if (freezable_by_creator) {
            ...
            let libra2_token = &mut Libra2Token[libra2_token_addr];
            let transfer_ref = object::generate_transfer_ref(&constructor_ref);
            libra2_token.transfer_ref.fill(transfer_ref);
        };
    ...
    public entry fun freeze_transfer<T: key>(creator: &signer, token: Object<T>) acquires Libra2Collection, Libra2Token {
        ...
        assert!(
            are_collection_tokens_freezable(token::collection_object(token))
                && libra2_token.transfer_ref.is_some(),
            error::permission_denied(EFIELD_NOT_MUTABLE),
        );
        object::disable_ungated_transfer(libra2_token.transfer_ref.borrow());
    }
    ...
    public entry fun unfreeze_transfer<T: key>(
        ...
        object::enable_ungated_transfer(libra2_token.transfer_ref.borrow());
    }
    ...
    public entry fun set_description<T: key>(
        ...
        let libra2_token = authorized_borrow(&token, creator);
        token::set_description(libra2_token.mutator_ref.borrow(), description);
    }
    ...
    public entry fun set_name<T: key>(
        ...
        let libra2_token = authorized_borrow(&token, creator);
        token::set_name(libra2_token.mutator_ref.borrow(), name);
    }
    ...
    public entry fun set_uri<T: key>(
        ...
        let libra2_token = authorized_borrow(&token, creator);
        token::set_uri(libra2_token.mutator_ref.borrow(), uri);
    }
    ...
    inline fun borrow_collection<T: key>(token: &Object<T>): &Libra2Collection {
        ...
        &Libra2Collection[collection_address]
    }
