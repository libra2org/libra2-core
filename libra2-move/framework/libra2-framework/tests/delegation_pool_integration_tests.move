...
module libra2_framework::delegation_pool_integration_tests {
    ...
    public entry fun test_inactive_validator_can_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_LBT, false, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator, 9900 * ONE_LBT_+ 1);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_LBT, true, true);
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_LBT, true, false);

        // Add more stake to exceed max. This should fail.
        mint_and_add_stake(validator_2, 9900 * ONE_LBT_+ 1);
    }
    ...
    public entry fun test_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_LBT, true, true);
    ...
    public entry fun test_active_validator_with_pending_inactive_stake_cannot_add_stake_if_exceeding_max_allowed(
        ...
        dp::unlock(validator, pool_address, 50 * ONE_LBT);
        stake::assert_validator_state(pool_address, 50 * ONE_LBT, 0, 0, 50 * ONE_LBT, 0);

        // Add 9900 LBT_+ 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 LBT_+ 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator, 9900 * ONE_LBT_+ 1);
    }
    ...
    public entry fun test_pending_inactive_cannot_add_stake_if_exceeding_max_allowed(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_LBT, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_LBT, true, true);
        ...
        // Add 9900 LBT_+ 1 more. Total stake is 50 (active) + 50 (pending_inactive) + 9900 LBT_+ 1 > 10000 so still exceeding max.
        mint_and_add_stake(validator_1, 9900 * ONE_LBT_+ 1);
    }
    ...
    public entry fun test_end_to_end(
        ...
        stake::mint(validator, 900 * ONE_LBT);
        dp::add_stake(validator, pool_address, 100 * ONE_LBT);
        assert!(coin::balance<Libra2Coin>(validator_address) == 800 * ONE_LBT, 2);
        stake::assert_validator_state(pool_address, 100 * ONE_LBT, 0, 100 * ONE_LBT, 0, 0);
        ...
        stake::assert_validator_state(pool_address, 201 * ONE_LBT, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_LBT);
        ...
        dp::withdraw(validator, pool_address, 50 * ONE_LBT);
        ...
        dp::withdraw(validator, pool_address, 51 * ONE_LBT);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_LBT, false, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_LBT, false, false);
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_LBT, false, true);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_LBT, false, false);
        ...
        mint_and_add_stake(validator_2, 100 * ONE_LBT);
        stake::assert_validator_state(validator_2_address, 200 * ONE_LBT, 0, 0, 0, 0);
    }
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator_2, ONE_LBT);
    }
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        let (_sk, pk, pop) = generate_identity();
        initialize_test_validator(&pk, &pop, validator, 100 * ONE_LBT, true, false);
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit_in_multiple_epochs(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(validator_address, 100 * ONE_LBT, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 110 * ONE_LBT, 0, 0, 0, 0);
        end_epoch();
        stake::assert_validator_state(validator_address, 121 * ONE_LBT, 0, 0, 0, 0);
        // Add more than 50% limit. The following line should fail.
        mint_and_add_stake(validator, 99 * ONE_LBT);
    }
    ...
    public entry fun test_active_validator_cannot_add_more_stake_than_limit(
        ...
        mint_and_add_stake(validator, 50 * ONE_LBT_+ 1);
    }
    ...
    public entry fun test_active_validator_unlock_partial_stake(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_LBT);
        stake::assert_validator_state(validator_address, 50 * ONE_LBT, 0, 0, 50 * ONE_LBT, 0);
        ...
        stake::assert_validator_state(validator_address, 55 * ONE_LBT, 55 * ONE_LBT, 0, 0, 0);
    ...
    public entry fun test_active_validator_can_withdraw_all_stake_and_rewards_at_once(
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_LBT, 0, 0, 0, 0);
        ...
        dp::unlock(validator, validator_address, 101 * ONE_LBT);
        stake::assert_validator_state(validator_address, 0, 0, 0, 101 * ONE_LBT, 0);
    ...
    public entry fun test_active_validator_unlocking_more_than_available_stake_should_cap(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 200 * ONE_LBT);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        let validator_address = dp::get_owned_pool_address(signer::address_of(validator));
        dp::unlock(validator, validator_address, 100 * ONE_LBT);
        ...
        dp::withdraw(validator, validator_address, 200 * ONE_LBT);
    ...
    public entry fun test_active_validator_can_reactivate_pending_inactive_stake(
        ...
        dp::reactivate_stake(validator, validator_address, 50 * ONE_LBT);
        stake::assert_validator_state(validator_address, 100 * ONE_LBT, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_reactivate_more_than_available_pending_inactive_stake_should_cap(
        ...
        dp::unlock(validator, validator_address, 50 * ONE_LBT);
        stake::assert_validator_state(validator_address, 50 * ONE_LBT, 0, 0, 50 * ONE_LBT, 0);
        dp::reactivate_stake(validator, validator_address, 51 * ONE_LBT);
        stake::assert_validator_state(validator_address, 100 * ONE_LBT, 0, 0, 0, 0);
    }
    ...
    public entry fun test_active_validator_leaves_staking_but_still_has_a_lockup(
        ...
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator, 100 * ONE_LBT, true, false);
        ...
        stake::assert_validator_state(validator_address, 100 * ONE_LBT, 0, 0, 0, 1);
        end_epoch();
        ...
        stake::assert_validator_state(validator_address, 101 * ONE_LBT, 0, 0, 0, 1);
    ...
    public entry fun test_pending_inactive_validator_does_not_count_in_increase_limit(
        ...
        mint_and_add_stake(validator_1, 51 * ONE_LBT);
    }
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100 * ONE_LBT, false, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_LBT, false, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 100 * ONE_LBT, false, false);
        ...
        stake::assert_validator_state(validator_1_address, 100 * ONE_LBT, 0, 0, 0, 0);
        stake::assert_validator_state(validator_2_address, 100 * ONE_LBT, 0, 0, 0, 1);
        ...
        stake::assert_validator_state(validator_1_address, 101 * ONE_LBT, 0, 0, 0, 0);
        ...
        stake::assert_validator_state(validator_2_address, 101 * ONE_LBT, 0, 0, 0, 1);
        assert!(stake::get_validator_state(validator_3_address) == VALIDATOR_STATUS_ACTIVE, 10);
        stake::assert_validator_state(validator_3_address, 100 * ONE_LBT, 0, 0, 0, 1);

        // Validators without enough stake will be removed.
        dp::unlock(validator_1, validator_1_address, 50 * ONE_LBT);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        mint_and_add_stake(validator, 100 * ONE_LBT);
        let pool_address = dp::get_owned_pool_address(signer::address_of(validator));
        stake::assert_validator_state(pool_address, 100 * ONE_LBT, 0, 0, 0, 0);
        ...
        dp::unlock(validator, pool_address, 100 * ONE_LBT);
        stake::assert_validator_state(pool_address, 0, 0, 0, 100 * ONE_LBT, 0);
        timestamp::fast_forward_seconds(LOCKUP_CYCLE_SECONDS);
        end_epoch();

        // Withdraw stake + rewards.
        stake::assert_validator_state(pool_address, 0, 101 * ONE_LBT, 0, 0, 0);
        dp::withdraw(validator, pool_address, 101 * ONE_LBT);
        assert!(coin::balance<Libra2Coin>(signer::address_of(validator)) == 101 * ONE_LBT, 1);
    ...
    public entry fun test_staking_validator_index(
        ...
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100 * ONE_LBT, false, false);
        initialize_test_validator(&pk_3, &pop_3, validator_3, 100 * ONE_LBT, false, false);
        initialize_test_validator(&pk_4, &pop_4, validator_4, 100 * ONE_LBT, false, false);
        initialize_test_validator(&pk_5, &pop_5, validator_5, 100 * ONE_LBT, false, false);
