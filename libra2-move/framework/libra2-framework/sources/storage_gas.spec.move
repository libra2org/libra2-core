spec libra2_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@libra2_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@libra2_framework);
    }
    ...
    spec set_config(libra2_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(libra2_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@libra2_framework);
        aborts_if exists<StorageGas>(@libra2_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@libra2_framework);
        ensures exists<StorageGas>(@libra2_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@libra2_framework);
        aborts_if !exists<StorageGas>(@libra2_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@libra2_framework);
    }
