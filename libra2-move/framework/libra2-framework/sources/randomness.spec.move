spec libra2_framework::randomness {

    spec module {
        use libra2_framework::chain_status;
        pragma verify = true;
        invariant [suspendable] chain_status::is_operating() ==> exists<PerBlockRandomness>(@libra2_framework);
    ...
    spec initialize(framework: &signer) {
        ...
        aborts_if framework_addr != @libra2_framework;
    }

    spec on_new_block(vm: &signer, epoch: u64, round: u64, seed_for_new_block: Option<vector<u8>>) {
        use std::signer;
        aborts_if signer::address_of(vm) != @vm;
        ensures exists<PerBlockRandomness>(@libra2_framework) ==> global<PerBlockRandomness>(@libra2_framework).seed == seed_for_new_block;
        ensures exists<PerBlockRandomness>(@libra2_framework) ==> global<PerBlockRandomness>(@libra2_framework).epoch == epoch;
        ensures exists<PerBlockRandomness>(@libra2_framework) ==> global<PerBlockRandomness>(@libra2_framework).round == round;
    }

    spec next_32_bytes(): vector<u8> {
        use std::hash;
        include NextBlobAbortsIf;
        let input = b"LIBRA2_RANDOMNESS";
        let randomness = global<PerBlockRandomness>(@libra2_framework);
    ...
    spec schema NextBlobAbortsIf {
        ...
        aborts_if !exists<PerBlockRandomness>(@libra2_framework);
    }
    ...
    spec permutation(n: u64): vector<u64> {
        ...
        // aborts_if n > 1 && !exists<PerBlockRandomness>(@libra2_framework);
    }
