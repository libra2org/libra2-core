...
module libra2_framework::storage_gas {
    ...
    public(friend) fun set_config(libra2_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_libra2_framework(libra2_framework);
        *borrow_global_mut<StorageGasConfig>(@libra2_framework) = config;
    }
    ...
    public fun initialize(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        assert!(
            !exists<StorageGasConfig>(@libra2_framework),
        ...
        move_to(libra2_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@libra2_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(libra2_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@libra2_framework);
        let gas = borrow_global_mut<StorageGas>(@libra2_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@libra2_framework);
