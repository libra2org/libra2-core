module libra2_framework::system_addresses {
    use std::error;
    use std::signer;
    ...
    const ENOT_LIBRA2_FRAMEWORK_ADDRESS: u64 = 3;
    ...
    public fun assert_libra2_framework(account: &signer) {
        assert!(
            is_libra2_framework_address(signer::address_of(account)),
            error::permission_denied(ENOT_LIBRA2_FRAMEWORK_ADDRESS),
        )
    }
    ...
    public fun is_framework_reserved_address(addr: address): bool {
        is_libra2_framework_address(addr) ||
            addr == @0x2 ||
            addr == @0x3 ||
            addr == @0x4 ||
            addr == @0x5 ||
    ...
    public fun is_libra2_framework_address(addr: address): bool {
