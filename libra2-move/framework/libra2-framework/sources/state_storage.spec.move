spec libra2_framework::state_storage {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, the resources for tracking state storage usage and gas
    /// parameters must exist for the Libra2 framework address.
    /// Criticality: Critical
    /// Implementation: The initialize function ensures only the Libra2 framework address can call it.
    ...
    /// storage usage will be moved under the Libra2 framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the libra2_framework account.
    ...
    /// usage will be moved under the Libra2 framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the libra2_framework account.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-5.3]
        invariant [suspendable] chain_status::is_operating() ==> exists<StateStorageUsage>(@libra2_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<GasParameter>(@libra2_framework);
    }
    ...
    spec initialize(libra2_framework: &signer) {
        ...
        /// [high-level-req-3]
        aborts_if exists<StateStorageUsage>(@libra2_framework);
        ensures exists<StateStorageUsage>(@libra2_framework);
        let post state_usage = global<StateStorageUsage>(@libra2_framework);
        /// [high-level-req-2]
    ...
    spec on_new_block(epoch: u64) {
        ...
        aborts_if false;
        ensures epoch == global<StateStorageUsage>(@libra2_framework).epoch;
    }
