spec libra2_framework::coin {
    ...
    /// Can only be updated by `@libra2_framework`.
    spec allow_supply_upgrades(_libra2_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@libra2_framework)) {
            let map = global<CoinConversionMap>(@libra2_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@libra2_framework`.
    ...
    // `account` must be `@libra2_framework`.
    ...
    monitor_supply: bool,
        ...
        aborts_if monitor_supply && !exists<aggregator_factory::AggregatorFactory>(@libra2_framework);
    ...
    parallelizable: bool,
        ...
        aborts_if monitor_supply && parallelizable
            && !exists<aggregator_factory::AggregatorFactory>(@libra2_framework);
