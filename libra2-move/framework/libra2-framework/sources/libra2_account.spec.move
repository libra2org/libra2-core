spec libra2_framework::libra2_account {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the creation of an Libra2 account the following rules should hold: (1) the authentication key
    /// should be 32 bytes in length, (2) an Libra2 account should not already exist for that authentication key, and (3)
    ...
    ///
    /// No.: 2
    /// Requirement: After creating an Libra2 account, the account should become registered to receive Libra2Coin.
    /// Criticality: Critical
    ...
    ///
    /// No.: 7
    /// Requirement: When performing a batch transfer of Libra2 Coin and/or a batch transfer of a custom coin type, it
    ...
    spec batch_transfer(source: &signer, recipients: vector<address>, amounts: vector<u64>) {
        ...
        aborts_if exists i in 0..len(recipients):
                !account::spec_exists_at(recipients[i]) && (recipients[i] == @vm_reserved || recipients[i] == @libra2_framework || recipients[i] == @libra2_token);
        ...
        // guid properties
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<Libra2Coin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        aborts_if exists i in 0..len(recipients):
            account::spec_exists_at(recipients[i]) && !exists<coin::CoinStore<Libra2Coin>>(recipients[i]) && global<account::Account>(recipients[i]).guid_creation_num + 2 > MAX_U64;
    }
    ...
    spec schema CreateAccountTransferAbortsIf {
        ...
        aborts_if !account::spec_exists_at(to) && (to == @vm_reserved || to == @libra2_framework || to == @libra2_token);
    }
