spec libra2_framework::staking_config {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: The ability to initialize the staking config and staking rewards resources, as well as the ability to
    /// update the staking config and staking rewards should only be available to the Libra2 framework account.
    /// Criticality: Medium
    ...
    /// update_voting_power_increase_limit functions, which ensure that the signer is libra2_framework using the
    /// assert_libra2_framework function.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StakingConfig>(@libra2_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StakingRewardsConfig>(@libra2_framework);
        pragma verify = true;
    ...
    spec get_reward_rate(config: &StakingConfig): (u64, u64) {
        ...
        ensures (features::spec_periodical_reward_rate_decrease_enabled() &&
            (global<StakingRewardsConfig>(@libra2_framework).rewards_rate.value as u64) != 0) ==>
                result_1 <= MAX_REWARDS_RATE && result_2 <= MAX_U64;
    }

    spec reward_rate(): (u64, u64) {
        let config = global<StakingConfig>(@libra2_framework);
        aborts_if !exists<StakingConfig>(@libra2_framework);
        include StakingRewardsConfigRequirement;
        ensures (features::spec_periodical_reward_rate_decrease_enabled() &&
            (global<StakingRewardsConfig>(@libra2_framework).rewards_rate.value as u64) != 0) ==>
    ...
    spec calculate_and_save_latest_epoch_rewards_rate(): FixedPoint64 {
        pragma verify_duration_estimate = 120;
        aborts_if !exists<StakingRewardsConfig>(@libra2_framework);
    ...
    spec update_required_stake(
        ...
        aborts_if minimum_stake > maximum_stake || maximum_stake == 0;
        aborts_if !exists<StakingConfig>(@libra2_framework);
        ensures global<StakingConfig>(@libra2_framework).minimum_stake == minimum_stake &&
            global<StakingConfig>(@libra2_framework).maximum_stake == maximum_stake;
    }
    ...
    spec update_recurring_lockup_duration_secs(
        ...
        aborts_if !exists<StakingConfig>(@libra2_framework);
        ensures global<StakingConfig>(@libra2_framework).recurring_lockup_duration_secs == new_recurring_lockup_duration_secs;
    }
    ...
    spec update_rewards_rate(
        ...
        let post staking_config = global<StakingConfig>(@libra2_framework);
    ...
    spec update_rewards_config(
        ...
        /// [high-level-req-1.6]
        aborts_if addr != @libra2_framework;
        aborts_if global<StakingRewardsConfig>(@libra2_framework).rewards_rate_period_in_secs != rewards_rate_period_in_secs;
        include StakingRewardsConfigValidationAbortsIf;
        aborts_if !exists<StakingRewardsConfig>(addr);
        let post staking_rewards_config = global<StakingRewardsConfig>(@libra2_framework);
    ...
    spec update_voting_power_increase_limit(
        ...
        aborts_if !exists<StakingConfig>(@libra2_framework);
        ensures global<StakingConfig>(@libra2_framework).voting_power_increase_limit == new_voting_power_increase_limit;
    }
    ...
    spec schema StakingRewardsConfigEnabledRequirement {
        requires exists<StakingRewardsConfig>(@libra2_framework);
        let staking_rewards_config = global<StakingRewardsConfig>(@libra2_framework);
