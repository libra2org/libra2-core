...
module libra2_framework::randomness_config_seqnum {
    ...
    public fun initialize(framework: &signer) {
        system_addresses::assert_libra2_framework(framework);
        if (!exists<RandomnessConfigSeqNum>(@libra2_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfigSeqNum {
        ...
        if (config_buffer::does_exist<RandomnessConfigSeqNum>()) {
            ...
            if (exists<RandomnessConfigSeqNum>(@libra2_framework)) {
                *borrow_global_mut<RandomnessConfigSeqNum>(@libra2_framework) = new_config;
            } else {
