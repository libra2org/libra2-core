...
module libra2_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use libra2_framework::event::emit;
    use libra2_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@libra2_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@libra2_framework)) {
            borrow_global<DKGState>(@libra2_framework).in_progress
        } else {
            option::none()
        }
    }
