module libra2_framework::state_storage {
    ...
    friend libra2_framework::storage_gas;
    ...
    public(friend) fun initialize(libra2_framework: &signer) {
        ...
        assert!(
            !exists<StateStorageUsage>(@libra2_framework),
            error::already_exists(ESTATE_STORAGE_USAGE)
        );
        move_to(libra2_framework, StateStorageUsage {
            epoch: 0,
            usage: Usage {
                items: 0,
                bytes: 0,
    ...
    public(friend) fun on_new_block(epoch: u64) acquires StateStorageUsage {
        ...
        let usage = borrow_global_mut<StateStorageUsage>(@libra2_framework);
    ...
    public(friend) fun current_items_and_bytes(): (u64, u64) acquires StateStorageUsage {
        ...
        let usage = borrow_global<StateStorageUsage>(@libra2_framework);
