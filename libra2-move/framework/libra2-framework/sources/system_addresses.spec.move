spec libra2_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the Libra2 Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_libra2_framework function ensures that the provided signer belongs to the
    /// @libra2_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotLibra2Framework).
    ///
    /// No.: 3
    ...
    spec assert_libra2_framework(account: &signer) {
        pragma opaque;
    ...
    spec schema AbortsIfNotLibra2Framework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @libra2_framework with error::PERMISSION_DENIED;
    }
