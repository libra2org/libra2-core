...
module libra2_framework::managed_coin {
    use std::string;
    use std::error;
    use std::signer;

    use libra2_framework::coin::{Self, BurnCapability, FreezeCapability, MintCapability, destroy_burn_cap,
    ...
    #[test(framework = @libra2_framework, source = @0xa11ce, destination = @0xb0b, mod_account = @0x1)]
    public entry fun test_end_to_end(
        ...
        libra2_framework::account::create_account_for_test(source_addr);
        libra2_framework::account::create_account_for_test(destination_addr);
        libra2_framework::account::create_account_for_test(signer::address_of(&mod_account));
        aggregator_factory::initialize_aggregator_factory_for_test(&mod_account);
        libra2_framework::coin::create_coin_conversion_map(&framework);
    ...
    public entry fun fail_mint(
        ...
        libra2_framework::account::create_account_for_test(source_addr);
        libra2_framework::account::create_account_for_test(signer::address_of(&destination));
        libra2_framework::account::create_account_for_test(signer::address_of(&mod_account));
        aggregator_factory::initialize_aggregator_factory_for_test(&mod_account);
        libra2_framework::coin::create_coin_conversion_map(&framework);
