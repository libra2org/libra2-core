module libra2_framework::nonce_validation {
    ...
    public(friend) fun initialize(libra2_framework: &signer) {
        initialize_nonce_table(libra2_framework);
    }

    public entry fun initialize_nonce_table(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        if (!exists<NonceHistory>(@libra2_framework)) {
            let table = table::new();
            let nonce_history = NonceHistory {
                nonce_table: table,
                next_key: 0,
            };
            move_to<NonceHistory>(libra2_framework, nonce_history);
        };
    }
    ...
    public entry fun add_nonce_buckets(count: u64) acquires NonceHistory {
        assert!(exists<NonceHistory>(@libra2_framework), error::invalid_state(E_NONCE_HISTORY_DOES_NOT_EXIST));
        let nonce_history = &mut NonceHistory[@libra2_framework];
        for (i in 0..count) {
    ...
    fun check_if_nonce_exists_in_history(
        ...
        let bucket_index = sip_hash_from_value(&nonce_key) % NUM_BUCKETS;
        let nonce_history = &NonceHistory[@libra2_framework];
        if (nonce_history.nonce_table.contains(bucket_index)) {
