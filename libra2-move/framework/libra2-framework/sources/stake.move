...
module libra2_framework::stake {
    ...
    public fun get_pending_transaction_fee(): vector<u64> acquires PendingTransactionFee {
        ...
        let fee_table = &borrow_global<PendingTransactionFee>(@libra2_framework).pending_fee_by_validator;
    ...
    public fun initialize_pending_transaction_fee(framework: &signer) {
        ...
        if (!exists<PendingTransactionFee>(@libra2_framework)) {
    ...
    public(friend) fun record_fee(
        ...
        let pending_fee = borrow_global_mut<PendingTransactionFee>(@libra2_framework);
        let i = 0;
    ...
    public(friend) fun on_new_epoch(
    ) acquires Libra2CoinCapabilities, PendingTransactionFee, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@libra2_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@libra2_framework);
        ...
        if (exists<PendingTransactionFee>(@libra2_framework)) {
            let pending_fee_by_validator = &mut borrow_global_mut<PendingTransactionFee>(@libra2_framework).pending_fee_by_validator;
    ...
    ) acquires Libra2CoinCapabilities, PendingTransactionFee, StakePool, ValidatorConfig {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, Libra2CoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(vm = @0x0, libra2_framework = @0x1, validator_0 = @0x123, validator_1 = @0x234)]
    public entry fun test_transaction_fee(
        ...
        initialize_for_test(libra2_framework);
        initialize_pending_transaction_fee(libra2_framework);
        features::change_feature_flags_for_testing(libra2_framework, vector[features::get_distribute_transaction_fee_feature()], vector[]);
