module libra2_framework::account_abstraction {
    ...
    #[test_only]
    use libra2_framework::auth_data;
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@libra2_framework].auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DerivableRegisterValue::Empty,
        );
    }

    public entry fun initialize(libra2_framework: &signer) {
        system_addresses::assert_libra2_framework(libra2_framework);
        move_to(
            libra2_framework,
    ...
    inline fun dispatchable_derivable_authenticator_internal(): &BigOrderedMap<FunctionInfo, DerivableRegisterValue> {
        assert!(exists<DerivableDispatchableAuthenticator>(@libra2_framework), error::not_found(EDERIVABLE_AA_NOT_INITIALIZED));
        &DerivableDispatchableAuthenticator[@libra2_framework].auth_functions
   }
