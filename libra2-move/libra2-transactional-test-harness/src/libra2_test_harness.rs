...
use libra2_vm_environment::{environment::Libra2Environment, prod_configs::set_paranoid_type_checks};
...
/// Default private key mappings for special Libra2 accounts.
...
static PRECOMPILED_APTOS_FRAMEWORK_V2: Lazy<PrecompiledFilesModules> = Lazy::new(|| {
    compile_framework_sources(
        libra2_cached_packages::head_release_bundle()
            ...
            .unwrap()
            .iter()
            .filter(|f| !f.contains("libra2-move/framework/libra2-experimental/sources"))
            .cloned()
            .collect::<Vec<_>>(),
        None,
    )
...
static PRECOMPILED_APTOS_FRAMEWORK_V2_WITH_EXPERIMENTAL: Lazy<PrecompiledFilesModules> =
    Lazy::new(|| {
        ...
        // libra2-experimental can be using latest language features, while others cannot.
        // So we need to compile it twice, once without libra2-experimental, and once with.
        // (in the second pass, we need to provide both, in case libra2-experimental depends on other modules)

        let all_sources = libra2_cached_packages::head_release_bundle()
            .files()
            .unwrap();
...
impl AptosTestAdapter<'_> {
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&libra2_test_root_address(), None, None, None, None)
            .unwrap();
        ...
        self.run_transaction(Transaction::UserTransaction(txn))
            .expect("Failed to mint libra2 coin. This should not happen.");
    }
}
...
impl<'a> MoveTestAdapter<'a> for AptosTestAdapter<'a> {
    ...
    fn deserialize(&self, bytes: &[u8], layout: &MoveTypeLayout) -> Option<Value> {
        let environment = Libra2Environment::new(&self.storage);
        let code_storage = self.storage.as_libra2_code_storage(&environment);
