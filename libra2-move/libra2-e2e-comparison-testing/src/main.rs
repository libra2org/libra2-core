...
pub enum Cmd {
    ...
    Dump {
        /// Endpoint url to obtain the txn data, e.g. `https://api.mainnet.libra2.org/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.libra2.org/`
    ...
    Online {
        ...
        /// e.g. `https://api.mainnet.libra2.org/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.libra2.org/`
        /// and set the env variable X_API_KEY using the obtained key
        endpoint: String,
        /// Path to the dumped data
