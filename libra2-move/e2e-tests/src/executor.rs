...
use libra2_block_executor::{
    code_cache_global_manager::Libra2ModuleCacheManager, txn_commit_hook::NoOpTransactionCommitHook,
...
use libra2_validator_interface::{DebuggerStateView, RestDebuggerInterface};
use libra2_vm::{
    block_executor::{Libra2TransactionOutput, Libra2VMBlockExecutorWrapper},
    data_cache::AsMoveResolver,
    gas::make_prod_gas_meter,
    move_vm_ext::{Libra2MoveResolver, MoveVmExt, SessionExt, SessionId},
...
use libra2_vm_types::{
    module_and_script_storage::{module_storage::Libra2ModuleStorage, AsLibra2CodeStorage},
...
impl FakeExecutor {
    ...
    /// Creates a [`FakeExecutor`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn from_remote_state(network_url: Libra2BaseUrl, txn_id: u64) -> Self {
    ...
    pub fn execute_transaction_with_gas_profiler(
        ...
        let env = Libra2Environment::new(&self.state_store);
        let vm = Libra2VM::new(&env, self.get_state_view());

        let resolver = self.state_store.as_move_resolver();
        let code_storage = self.get_state_view().as_libra2_code_storage(&env);
    ...
    pub fn validate_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        ...
        vm.validate_transaction(
            txn,
            &self.state_store,
            &self.state_store.as_libra2_code_storage(&env),
        )
    }
    ...
    pub fn exec_func_record_running_time(
        ...
        let module_storage = self.state_store.as_libra2_code_storage(&env);
...
fn finish_session_assert_no_modules(
    session: SessionExt<impl Libra2MoveResolver>,
