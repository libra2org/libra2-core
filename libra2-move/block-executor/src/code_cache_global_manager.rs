...
use libra2_gas_schedule::gas_feature_versions::RELEASE_V1_34;
use libra2_types::{
    block_executor::{
...
impl Libra2ModuleCacheManager {
    ...
    /// is ready for execution and updates states. If fails, [Libra2ModuleCacheManagerGuard::None]
    ...
    fn try_lock_inner(
        ...
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<Libra2ModuleCacheManagerGuard<'_>, VMStatus> {
        ...
        Ok(match self.inner.try_lock() {
            Some(mut guard) => {
                ...
                Libra2ModuleCacheManagerGuard::Guard { guard }
            },
            None => {
                ...
                Libra2ModuleCacheManagerGuard::None {
...
/// A guard that can be acquired from [Libra2ModuleCacheManager]. Variants represent successful and
/// no-successful lock acquisition.
pub enum Libra2ModuleCacheManagerGuard<'a> {
    /// Holds the guard to the [Libra2ModuleCacheManager], and has exclusive access to it.
    Guard {
        guard: MutexGuard<
            'a,
...
impl Libra2ModuleCacheManagerGuard<'_> {
    ...
    /// A guard in [Libra2ModuleCacheManagerGuard::None] state with empty module cache and default
    ...
    /// A guard in [Libra2ModuleCacheManagerGuard::None] state with empty module cache and the
    ...
    pub(crate) fn none_for_state_view(state_view: &impl StateView) -> Self {
        Libra2ModuleCacheManagerGuard::None {
            environment: Libra2Environment::new(state_view),
            module_cache: GlobalModuleCache::empty(),
        }
    }
}
...
fn prefetch_libra2_framework(
    state_view: &impl StateView,
    guard: &mut Libra2ModuleCacheManagerGuard,
) -> Result<(), PanicError> {
    let code_storage = state_view.as_libra2_code_storage(guard.environment());
...
mod test {
    ...
    fn test_prefetch_existing_libra2_framework() {
        ...
        let mut guard = Libra2ModuleCacheManagerGuard::none_for_state_view(&state_view);
        assert_eq!(guard.module_cache().num_modules(), 0);

        let result = prefetch_libra2_framework(&state_view, &mut guard);
        assert!(result.is_ok());
    ...
    fn test_try_lock_inner_single_thread() {
        let manager = Libra2ModuleCacheManager::new();
        ...
        assert!(matches!(guard, Libra2ModuleCacheManagerGuard::Guard { .. }));
    }

    #[test]
    fn test_try_lock_inner_multiple_threads() {
        let manager = Arc::new(Libra2ModuleCacheManager::new());
        ...
        for _ in 0..num_threads {
            let handle = std::thread::spawn({
                ...
                move || {
                    ...
                    if matches!(guard, Libra2ModuleCacheManagerGuard::Guard { .. }) {
                        1
                    } else {
                        0
                    }
