...
/// - Account address refers to a specific destination on the Libra2 network. The address dictates the destination and source of a specific amount of assets exchanged by two parties on the blockchain.
...
/// ### API
///
/// - An **Application Programming Interface (API)(** is a set of protocols and tools that allow users to interact with Libra2 blockchain nodes and client networks via external applications. Libra2 offers a REST API for this purpose. See the [Libra2 API reference](https://docs.libra2.org/nodes/libra2-api-spec#/) documentation and [Use the Libra2 API](../integration/fullnode-rest-api.md) for more details.
///
/// ### LBT
///
/// **Libra2 token (LBT)** is the Libra2 blockchain native token used for paying network and transaction fees.
///
/// ### Libra2
///
/// **Libra2** is a Layer 1 blockchain for everyone. It uses the Move programming language and launched its mainnet on 2022-10-17 to redefine the web3 user experience. The Libra2 blockchain is dedicated to creating better user experiences through increased speed, security, scalability, reliability and usability with low transaction costs.  The word "Libra2" means "The People" in the Ohlone language. See the [Libra2 White Paper](../aptos-white-paper/index.md) for more details.
///
/// ### Libra2BFT
///
...
/// ### Libra2 Blockchain
///
/// - The **Libra2 blockchain** is a ledger of immutable transactions agreed upon by the validators on the Libra2 network (the network of validators).
///
/// ### Libra2 Name Service (ANS)
///
/// - The **Libra2 Name Service (ANS)** is a decentralized naming address service for the Libra2 blockchain. An Libra2 name is a human-readable *.apt* domain name that is used in place of a public key, for example *love.apt*.
/// - This service also allows users to register subdomain names in addition to the registered domain. Find out more at: [Libra2.org](https://www.libra2.org)
///
/// ### Libra2-core
///
/// **Libra2-core** is the open source technology on which the Libra2 Payment Network runs. Libra2-core contains software for
///
...
/// ### Libra2 Ecosystem
///
/// - **Libra2 ecosystem** refers to various components of the Libra2 blockchain network and their interactions.  The Libra2 ecosystem includes the community, community-driven projects, and events. See [Contribute to the Libra2 Ecosystem](../community/index.md) for all possible ways to join Libra2.
///
/// ### Libra2 Explorer
///
/// - The **[Libra2 Explorer](https://explorer.libra2.org/)** is an interface that helps users examine details of the Libra2 blockchain, including account information, validators, and transactions.
/// - The Libra2 Explorer help users validate their work in Libra2 wallets and other tools in the blockchain. Find more details at [Use the Libra2 Explorer](../guides/explore-libra2.md).
///
/// ### Libra2 Framework
/// The **Libra2 Framework** defines the public API for blockchain updates and the structure of on-chain data. It defines the business logic and access control for the three key pillars of Libra2 functionality: payments, treasury, and on-chain governance. It is implemented as a set of modules written in the Move programming language and stored on-chain as Move bytecode.
///
/// ### Libra2 Node
/// An **Libra2 node** is a peer entity of the Libra2 network that tracks the state of the Libra2 blockchain. There are two types of Libra2 nodes, [validators](#validator) and [fullnodes](#fullnode)).
///
/// ### Libra2 Protocol
///
/// - **Libra2 protocol** is the specification of how transactions are submitted, ordered, executed, and recorded within the Libra2 network.
///
/// ### Libra2Account
///
/// - A **`Libra2Account`** is a Move resource that holds all the administrative data associated with an account, such as sequence number, balance, and authentication key.
/// - A **`Libra2Account`** is the only resource that every account is guaranteed to contain.
///
/// ### Libra2Account module
///
/// - **The Libra2Account module** is a Move module that contains the code for manipulating the administrative data held in a particular `Libra2Account.T` resource.
/// - Code for checking or incrementing sequence numbers, withdrawing or depositing currency, and extracting gas deposits is included in the Libra2Account module.
///
/// ### Libra2 devnet
///
/// - See [devnet](#devnet).
...
/// - The Libra2 blockchain uses Libra2BFT, a consensus protocol based on [Jolteon](#Jolteon).
/// - BFT algorithms typically operate with a number of entities, collectively holding N votes (which are called "validators" in the Libra2 network's application of the system).
...
/// ### CLI
///
/// - **Command line interface** refers to the Libra2 CLI used for developing on the Libra2 blockchain, operating nodes, and debugging issues. Find out more at [Use the Libra2 CLI](../tools/libra2-cli-tool/use-libra2-cli.md).
///
/// ### Client
///
/// - **Client** is software that receives information from the blockchain and manages transactions. Clients interact with the blockchain through the Libra2 nodes.
///
/// ### Code labs
///
...
/// ### devnet
///
/// - The **Libra2 devnet** is a publicly deployed instance of the Libra2 network that runs using a set of validator test nodes.
/// - The devnet is a demonstration of the Libra2 network that is built for experimenting with new ideas
...
/// - More specifically, the Libra2 network uses the PureEdDSA scheme over the Ed25519 curve, as defined in RFC 8032.
///
/// ### Event
///
...
/// - In the Libra2 protocol, events provide evidence that the successful execution of a transaction resulted in a specific effect. The `DepositEvent` (in the above example) allows the recipient to confirm that a payment was received into their account.
/// - Events are persisted on the blockchain and are used to answer queries by [clients](#client).
///
/// ### Execution
///
/// - **Execution** in the Libra2 blockchain is an Libra2 node component that manages the block of transactions. The execution component stores successful transactions.
///
/// ### Expiration Time
///
...
/// ### Faucet
///
/// - **Faucet** is a service that mints LBT_on devnet and testnet. LBT_on these networks has no real world value, it is only for development purposes.
/// - You can use the faucet in a few different ways:
///   - With the [Libra2 CLI](../tools/libra2-cli-tool/use-libra2-cli.md#fund-an-account-with-the-faucet).
...
/// ### Fungible Asset
///
/// - A **fungible asset** is an asset, such as a currency, share, in-game resource, etc., that is interchangeable with another identical asset without any loss in its value. For example, LBT_is a fungible asset because you can exchange one LBT_for another.
...
/// ### Gas
///
/// - **Gas** is a way to pay for computation and storage on a blockchain network. All transactions on the Libra2 network cost a certain amount of gas.
...
/// ### Indexer
///
/// - **[Indexer](../integration/indexing.md)** is the component of Libra2 that retrieves, processes, and efficiently stores raw data in the database to provide speedy access to the Libra2 blockchain state.
///
/// ## L
///
/// ### Leader
...
/// - Transactions in the mempool of a validator are added from the JSON-RPC Service of the current node and from the mempool of other Libra2 nodes.
...
/// - The Libra2 network treats the entire blockchain as a single data structure that records the history of transactions and states over time.
...
/// - Move is a safe and secure programming language for web3 that emphasizes access control and scarcity. It is the programming language used to build the Libra2 blockchain. You can read more about it in [Move on Libra2](../move/move-on-libra2.md).
///
/// ### Move Bytecode
///
...
/// ### Node
///
/// - A **node** is a peer entity of the Libra2 network that tracks the state of the Libra2 blockchain.
/// - An Libra2 node consists of logical components. [Mempool](#mempool), [consensus](#consensus), and the [virtual machine](#virtual-machine) are examples of node components.
///
/// ### Nonce
///
...
///
/// - **Open-source community** is a term used for a group of developers who work on open-source software. If you're reading this glossary, then you are part of the Libra2 project's developer community.
///
/// ## P
///
/// ### Proof
...
/// ### SDKs
///
/// - Libra2 **software development kits (SDKs)** are sets of tools that enable a developer to quickly create a custom app on the Libra2 platform. Find out more at [Use the Libra2 SDKs](../sdks/index.md).
///
/// ### Sequence Number
///
...
/// ### Table
///
/// - A [**table**](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/doc/table.md) implements the Table type and in Libra2 is used to store information as key-value data within an account at large scale.
///
/// See [`table.move`](https://github.com/libra2org/libra2-core/blob/main/libra2-move/framework/libra2-stdlib/sources/table.move) for the associated Libra2 source file.
///
/// ### Testnet
///
/// - **Testnet** describes the Libra2 network that is not fully functional yet more stable than devnet; it is an alternative network to mainnet to be used for testing.
///
/// ### Tokens
///
...
/// - A **validator** is an entity of the Libra2 ecosystem that validates on the Libra2 blockchain. It receives requests from clients and runs consensus, execution, and storage.
...
/// ### Validator Nodes
///
/// - **Validator nodes** are a unique class of fullnodes that take part in consensus, specifically a Byzantine Fault Tolerance (BFT) consensus protocol in Libra2. Validators agree upon transactions to be added to the Libra2 blockchain as well as the order in which they are added.
///
/// ### Version
///
...
///
/// An Libra2 transaction is **well formed** if each of the following conditions are true for the transaction:
///
