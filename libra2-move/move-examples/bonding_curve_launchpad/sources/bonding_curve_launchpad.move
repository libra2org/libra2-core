module bonding_curve_launchpad::bonding_curve_launchpad {
    ...
    use libra2_framework::fungible_asset::{Self, FungibleAsset, Metadata, TransferRef};
    ...
    /// Each created FA has transfers disabled, until the LBT reserves minimum threshold on
    ...
    /// Optionally, the participant can immediately perform an initial swap from LBT to FA.
    public entry fun create_fa_pair(
        ...
        // block the transfer of LBT to the creator.
        let transfer_ref = &borrow_global<FAController>(fa_address).transfer_ref;
        // Create the liquidity pair between LBT and the new FA. Include the initial creator swap, if needed.
    ...
    /// Swap from FA to LBT, or vice versa, through `liquidity_pair`.
