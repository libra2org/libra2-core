module bonding_curve_launchpad::bonding_curve_launchpad {
    ...
    /// Each created FA has transfers disabled, until the LBT_reserves minimum threshold on
    ...
    /// Optionally, the participant can immediately perform an initial swap from LBT_to FA.
    public entry fun create_fa_pair(
        ...
        // block the transfer of LBT_to the creator.
        let transfer_ref = &borrow_global<FAController>(fa_address).transfer_ref;
        // Create the liquidity pair between LBT_and the new FA. Include the initial creator swap, if needed.
