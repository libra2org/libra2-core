...
1. After each swap from LBT to FA, when the LBT reserves are increasing, a threshold is checked for whether the liquidity pair can **graduate** or not. The threshold is a pre-defined minimum amount of LBT that must exist in the reserves. Once this threshold is met during a swap, **graduation** begins.
   ...
   2. The reserves from the liquidity pair, both LBT and FA, are moved to an external, public third-party DEX as a new liquidity pair. In this case, the `libra2-move` FA DEX example, called swap. 
