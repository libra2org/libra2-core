...
module staking::commission_tests {
    ...
    // 1 LBT_= 5.34378710 USD with 8 decimals of precision
    ...
    fun test_distribution_with_no_debt(manager: &signer) {
        ...
        // Send LBT_to the commission contract.
        ...
        commission::distribute_commission(manager);
        assert!(commission::commission_owed() == 0);
        assert!(coin::balance<Libra2Coin>(signer::address_of(manager)) == expected_apt_amount);
        assert!(coin::balance<Libra2Coin>(OPERATOR) == expected_apt_amount);
    }
    ...
    fun test_distribution_with_debt(manager: &signer) {
        ...
        // Send LBT_to the commission contract. But not enough to cover the commission owed
        let expected_commission_usd = 50000;
        let expected_apt_amount = usd_to_apt(expected_commission_usd);
        mint_apt(expected_apt_amount / 2);
        ...
        assert!(coin::balance<Libra2Coin>(signer::address_of(manager)) == 0);
        assert!(coin::balance<Libra2Coin>(OPERATOR) == expected_apt_amount / 2);
    }
    ...
    fun mint_apt(amount: u64) {
        let (burn_cap, mint_cap) = libra2_coin::initialize_for_test(
            &account::create_signer_for_test(@libra2_framework));
        libra2_account::deposit_coins(@staking, coin::mint(amount, &mint_cap));
