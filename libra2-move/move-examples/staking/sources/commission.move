...
///
/// The commission rate is set in dollars and will be used to determine how much LBT the operator receives.
/// The commission is distributed to the operator and remaining amount to the manager. If there's not enough balance
/// to pay the commission, either commission rate is set too high or LBT price is low. In this case, the commission
...
module staking::commission {
    ...
    struct CommissionConfig has key {
        ...
        operator: address,
        /// The yearly commission rate in dollars. Will be used to determine how much LBT the operator receives.
        ...
        /// This can happen if the commission rate is set too high or LBT price is too low.
        commission_debt: u64
    }
    ...
    public entry fun distribute_commission(account: &signer) acquires CommissionConfig {
        ...
        let balance = coin::balance<Libra2Coin>(@staking);
        ...
        // If there's not enough balance to pay the commission, either commission rate is set too high or LBT price is low.
        // Otherwise, pay the operator the commission in LBT and send remaining balance to the manager.
        if (balance <= commission_in_apt) {
            ...
            // There can be rounding error here when converting from LBT to USD. If this is of concern, the amount of
            ...
            libra2_account::transfer(commission_signer, config.manager, surplus_balance);
        };

        let remaining_balance = coin::balance<Libra2Coin>(@staking);
        libra2_account::transfer(commission_signer, config.operator, remaining_balance);
    ...
    inline fun usd_to_apt(usd_amount: u64): u64 {
        ...
        // Amount in LBT octas = amount * number of octas in one LBT / LBT price.
    ...
    inline fun apt_to_usd(apt_amount: u64): u64 {
        let apt_price = oracle::get_apt_price();
        // Amount in USD = amount * LBT price / precision / number of octas in one LBT.
