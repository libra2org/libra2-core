...
/// Helper function to fetch the LBT balance of the specified account.
...
/// Reads the LBT balance of the test account and checks that it matches the expected value.
#[ignore]
...
/// Transfers 1 LBT from a newly created account to the existing test account
...
async fn transfer_to_existing_account() {
    ...
    let new_account =
        h.new_account_with_balance_and_sequence_number(10_0000_0000 /* 10 LBT */, 0);

    // Transfer 1 LBT to the existing account.
...
/// Attempts to transfer 1 LBT from the existing test account to a newly created account,
...
async fn transfer_from_existing_account() {
    ...
    // Transfer 1 LBT from the existing account to the new account.
    let status = h.run_entry_function(
        &existing_account,
        str::parse("0x1::coin::transfer").unwrap(),
        vec![TypeTag::from_str("0x1::libra2_coin::Libra2Coin").unwrap()],
