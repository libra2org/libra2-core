...
use libra2_rest_client::Libra2BaseUrl;
...
const TESTNET_ACCOUNT_LBT_BALANCE: u64 = 91_8290_3550;

/// Helper function to fetch the LBT_balance of the specified account.
...
/// Reads the LBT_balance of the test account and checks that it matches the expected value.
#[ignore]
...
async fn view_existing_account_balance() {
    ...
    let mut h = MoveHarness::new_with_remote_state(Libra2BaseUrl::Testnet, TESTNET_TXN_VERSION);
    ...
    assert_eq!(
        ...
        TESTNET_ACCOUNT_LBT_BALANCE
    )
}

/// Transfers 1 LBT_from a newly created account to the existing test account
...
async fn transfer_to_existing_account() {
    ...
    // Create a new account and fund it with 10 LBT.
    let new_account =
        h.new_account_with_balance_and_sequence_number(10_0000_0000 /* 10 LBT_*/, 0);

    // Transfer 1 LBT_to the existing account.
    ...
    // Verify that the recipient's balance has increased by 1 LBT.
    assert_eq!(
        get_account_apt_balance(&mut h, existing_account_addr),
        TESTNET_ACCOUNT_LBT_BALANCE + 1_0000_0000
    )
}

/// Attempts to transfer 1 LBT_from the existing test account to a newly created account,
...
async fn transfer_from_existing_account() {
    ...
    // Transfer 1 LBT_from the existing account to the new account.
    let status = h.run_entry_function(
        &existing_account,
        str::parse("0x1::coin::transfer").unwrap(),
        vec![TypeTag::from_str("0x1::libra2_coin::Libra2Coin").unwrap()],
