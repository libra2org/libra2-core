...
fn call_get_chain_id_from_libra2_framework(harness: &mut MoveHarness, account: &Account) -> u8 {
    let status = harness.run_entry_function(
        account,
        str::parse("0x1::chain_id_test::store_chain_id_from_libra2_framework").unwrap(),
        vec![],
        vec![],
    );
...
#[test]
fn test_chain_id_from_libra2_framework() {
    let mut harness = MoveHarness::new();
    let account = setup(&mut harness);

    assert_eq!(
        call_get_chain_id_from_libra2_framework(&mut harness, &account),
        4u8
    );
}
