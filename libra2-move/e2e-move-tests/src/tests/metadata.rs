...
use libra2_types::{
    ...
    vm::module_metadata::{
        RuntimeModuleMetadata, RuntimeModuleMetadataV1, LIBRA2_METADATA_KEY, LIBRA2_METADATA_KEY_V1,
    },
};
...
fn test_duplicate_entries() {
    let duplicate_same_version = || {
        let metadata = Metadata {
            key: LIBRA2_METADATA_KEY_V1.to_vec(),
    ...
    let duplicate_different_version = || {
        ...
        let metadata_v0 = Metadata {
            key: LIBRA2_METADATA_KEY.to_vec(),
