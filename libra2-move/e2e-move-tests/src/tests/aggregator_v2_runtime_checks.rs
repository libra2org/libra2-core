...
fn publish_test_package(h: &mut MoveHarness, libra2_framework_account: &Account) {
    let path_buf = common::test_dir_path("aggregator_v2.data/pack");
    assert_success!(h.publish_package_cache_building(libra2_framework_account, path_buf.as_path()));
}

fn create_test_txn(
    h: &mut MoveHarness,
    libra2_framework_account: &Account,
    name: &str,
) -> SignedTransaction {
...
fn run_entry_functions<F: Fn(ExecutionStatus)>(func_names: Vec<&str>, check_status: F) {
    ...
    let libra2_framework_account = h.libra2_framework_account();
    publish_test_package(&mut h, &libra2_framework_account);
    ...
    let txns = func_names
        .into_iter()
        .map(|name| create_test_txn(&mut h, &libra2_framework_account, name))
        .collect();
