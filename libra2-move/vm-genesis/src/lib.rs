...
use libra2_vm::{
    ...
    move_vm_ext::{
        convert_modules_into_write_ops, Libra2MoveResolver, GenesisMoveVm, GenesisRuntimeBuilder,
        SessionExt,
    },
};
use libra2_vm_types::{
...
pub fn encode_libra2_mainnet_genesis_transaction(
    ...
    let module_storage = state_view.as_libra2_code_storage(&genesis_runtime_environment);
    ...
    initialize_libra2_coin(&mut session, &module_storage, &mut traversal_context);
...
fn code_to_writes_for_publishing(
    ...
    let module_storage = genesis_state_view.as_libra2_code_storage(genesis_runtime_environment);
